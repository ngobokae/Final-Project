...
const EXECUTIVE_ROLE_LABEL = 'Executive Manager';
const SHEET_COL_COUNT = 10;

const formatRoleLabel = (role) => {
  const map = {
    admin: 'Administrator',
    executive: EXECUTIVE_ROLE_LABEL,
...
const getCenteredColumnRange = (colCount) => {
  const safeCount = Math.max(1, Math.min(colCount, SHEET_COL_COUNT));
  const startCol = Math.max(1, Math.floor((SHEET_COL_COUNT - safeCount) / 2) + 1);
  const endCol = startCol + safeCount - 1;
  return { startCol, endCol, startLetter: colIndexToLetter(startCol), endLetter: colIndexToLetter(endCol) };
};
...
const getExportMeta = () => {
  const fallback = {
    ...
    approvedRole: EXECUTIVE_ROLE_LABEL,
    isExecutive: false,
    showApprovedBy: true,
  };
  ...
  try {
    ...
    return {
      ...
      preparedRole: isExecutive ? EXECUTIVE_ROLE_LABEL : formatRoleLabel(user.role),
      approvedRole: EXECUTIVE_ROLE_LABEL,
      isExecutive,
      showApprovedBy: !isExecutive,
    };
  } catch {
    return { email: 'N/A', preparedRole: 'Staff', approvedRole: EXECUTIVE_ROLE_LABEL, isExecutive: false, showApprovedBy: true };
  }
};
...
const buildCsvSignatureBlock = (meta) => {
  ...
  const lines = ['', centerText('─'.repeat(60))];

  if (meta.showApprovedBy) {
    lines.push(
    ...
    lines.push(
      centerText('PREPARED BY'),
      '',
      centerText('Signature: ____________________'),
      centerText(meta.preparedRole),
      centerText('Date: __________________')
    );
  }
...
const styleExcelHeaderRow = (row, startCol = 1, colCount) => {
  row.height = 26;
  const endCol = startCol + (colCount ?? row.cellCount) - 1;
...
const styleExcelDataRows = (sheet, startRow, endRow, colCount, startCol = 1) => {
...
const autoSizeColumns = (sheet, colCount, startRow, endRow, startCol = 1) => {
  for (let c = startCol; c < startCol + colCount; c += 1) {
    let maxLen = 12;
    for (let r = startRow; r <= endRow; r += 1) {
...
const addExcelReportHeader = (sheet, title, report, logoBase64, workbook) => {
  const mergeEnd = colIndexToLetter(SHEET_COL_COUNT);
...
const addExcelSignatureBlock = (sheet, startRow, meta) => {
  ...
  const drawSignBox = (startCol, endCol, label, role) => {
    ...
    sheet.getCell(`${startCol}${dateRow}`).alignment = { horizontal: 'center' };
  ...
  if (meta.showApprovedBy) {
    drawSignBox('A', 'E', 'PREPARED BY', meta.preparedRole);
    drawSignBox('F', mergeEnd, 'APPROVED BY', meta.approvedRole);
  } else {
    drawSignBox('C', 'H', 'PREPARED BY', meta.preparedRole);
  }
...
export const buildExcelBuffer = async (report, title) => {
  ...
  if (!sections.length) {
    sheet.mergeCells(`A${currentRow}:${colIndexToLetter(SHEET_COL_COUNT)}${currentRow}`);
  ...
  sections.forEach((section) => {
    ...
    const { startCol } = getCenteredColumnRange(section.headers.length);
    const headerRow = sheet.getRow(currentRow);
    section.headers.forEach((header, idx) => {
      headerRow.getCell(startCol + idx).value = header;
    });
    styleExcelHeaderRow(headerRow, startCol, section.headers.length);
    ...
    section.body.forEach((rowValues) => {
      ...
      rowValues.forEach((val, idx) => {
        dataRow.getCell(startCol + idx).value = val;
      });
      currentRow += 1;
    });
    ...
    if (dataEnd >= dataStart) {
      styleExcelDataRows(sheet, dataStart, dataEnd, section.headers.length, startCol);
      autoSizeColumns(sheet, section.headers.length, headerRowNum, dataEnd, startCol);
    }
    currentRow += 1;
  });
...
const addPdfBanner = (doc, title, report, logoBase64) => {
  ...
  if (logoBase64) {
    try {
      doc.addImage(`data:image/png;base64,${logoBase64}`, 'PNG', margin, 14, 52, 52);
    } catch {
      /* logo optional */
    }
  }

  doc.setTextColor(...BRAND.dark.rgb);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(18);
  doc.text(COMPANY_NAME, centerX, 34, { align: 'center' });

  doc.setTextColor(...BRAND.red.rgb);
  doc.setFontSize(14);
  doc.text(title || getReportTypeLabel(report), centerX, 56, { align: 'center' });

  doc.setFont('helvetica', 'italic');
  doc.setFontSize(10);
  doc.setTextColor(...BRAND.muted.rgb);
  doc.text(REPORT_SUBTITLE, centerX, 72, { align: 'center' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.text(`Generated: ${formatExportDate()}  •  Period: ${getPeriodLabel(report)}`, centerX, 88, { align: 'center' });

  doc.setDrawColor(...BRAND.red.rgb);
  doc.setFillColor(...BRAND.red.rgb);
  doc.rect(margin, bannerHeight - 8, pageWidth - margin * 2, 4, 'F');
...
const addPdfSignatureBlock = (doc, y, meta) => {
  ...
  const gap = 20;
  const boxHeight = 88;
  const singleBoxWidth = Math.min(280, pageWidth - margin * 2);
  const dualBoxWidth = (pageWidth - margin * 2 - gap) / 2;
  ...
  const drawBox = (x, width, label, role) => {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...BRAND.muted.rgb);
    doc.text('Signature', x + width / 2, y + 38, { align: 'center' });

    doc.setDrawColor(...BRAND.muted.rgb);
    doc.line(x + 24, y + 48, x + width - 24, y + 48);

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(30, 41, 59);
    doc.text(role, x + width / 2, y + 62, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.text('Date: __________________', x + width / 2, y + 78, { align: 'center' });
  };

  if (meta.showApprovedBy) {
    drawBox(margin, dualBoxWidth, 'PREPARED BY', meta.preparedRole);
    drawBox(margin + dualBoxWidth + gap, dualBoxWidth, 'APPROVED BY', meta.approvedRole);
  } else {
    drawBox((pageWidth - singleBoxWidth) / 2, singleBoxWidth, 'PREPARED BY', meta.preparedRole);
  }
